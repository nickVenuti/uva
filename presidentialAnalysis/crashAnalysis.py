...
#Get tokens
#rawTokens=sd.tokenize(rawFileList)
...
sys.stdout.flush()
CoCo, TF, docTF = sd.coOccurence(tokens,6)
...
sys.stdout.flush()
startTime=time.time()
CVDict=sd.contextVectors(tokens, DSM, wordList, 6)
...
sys.stdout.flush()
preCrashCosine=sd.averageCosine(CVDict,preCrashFiles,wordList,1000)
...
sys.stdout.flush()
crashCosine=sd.averageCosine(CVDict,crashFiles,wordList,1000)
...
sys.stdout.flush()
postCrashCosine=sd.averageCosine(CVDict,postCrashFiles,wordList,1000)
pd.DataFrame(postCrashCosine).to_csv('./postCrash_cosine.csv')
print('finished sem density 3')
sys.stdout.flush()

#Get TF for each period
preCrashTF=sd.getPeriodTF(docTF,preCrashFiles)
...
#Get TF for each period
crashTF=sd.getPeriodTF(docTF,crashFiles)
...
#Get TF for each period
postCrashTF=sd.getPeriodTF(docTF,postCrashFiles)
...
tfList.sort(key=lamsda x:x[1], reverse = True)
...
sys.stdout.flush()
startTime=time.time()
preCrashKNN=sd.knnContextVector(CVDict,preCrashFiles,contextList,wordList,1000,5)
endTime=time.time()
...
sys.stdout.flush()
startTime=time.time()
crashKNN=sd.knnContextVector(CVDict,crashFiles,contextList,wordList,1000,5)
endTime=time.time()
...
sys.stdout.flush()
startTime=time.time()
postCrashKNN=sd.knnContextVector(CVDict,postCrashFiles,contextList,wordList,1000,5)
endTime=time.time()
