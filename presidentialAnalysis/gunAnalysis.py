...
sys.stdout.flush()
preSandyCosine=sd.averageCosine(CVDict,preSandyFiles,wordList,1000)
...
sys.stdout.flush()
postSandyCosine=sd.averageCosine(CVDict,postSandyFiles,wordList,1000)
pd.DataFrame(postSandyCosine).to_csv('./postSandy_cosine.csv')
print('finished sem density 2')
sys.stdout.flush()

#Get TF for each period
preSandyTF=sd.getPeriodTF(docTF,preSandyFiles)
...
#Get TF for each period
postSandyTF=sd.getPeriodTF(docTF,postSandyFiles)
...
sys.stdout.flush()
startTime=time.time()
preSandyKNN=sd.knnContextVector(CVDict,preSandyFiles,contextList,wordList,1000,5)
endTime=time.time()
...
sys.stdout.flush()
startTime=time.time()
postSandyKNN=sd.knnContextVector(CVDict,postSandyFiles,contextList,wordList,1000,5)
endTime=time.time()
